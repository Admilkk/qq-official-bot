...
import {
	...
	createMiddlewareReceiver,
} from '@/receivers/webhook'
import { createWebsocketReceiver } from '@/receivers/websocket'
import { Receiver } from '@/receiver'
import { DataPacket } from '@/types'

export const MAX_RETRY = 10
export type ResolveReceiver<
	T extends Receiver.ReceiveMode,
	M extends ApplicationPlatform = never,
> = T extends 'websocket'
...
> extends EventEmitter {
	...
	constructor(bot: QQBot<T, M>) {
		...
		switch (bot.config.mode) {
			case 'middleware':
				this.receiver = createMiddlewareReceiver(
					(bot.config as QQBot.Config<'middleware'>).application,
				) as ResolveReceiver<T, M>
				break
			case 'webhook':
				const { port, path } = bot.config as QQBot.Config<'webhook'>
				this.receiver = createWebhookReceiver(port, path) as ResolveReceiver<
					T,
					M
				>
				break
			...
			switch (data.eventType) {
				...
				case SessionEvents.DISCONNECT:
					...
					if (this.retry < (this.bot.config.maxRetry || MAX_RETRY)) {
						this.bot.logger.mark(
							'[CLIENT] 重新连接中，尝试次数：',
							this.retry + 1,
						)
						if (
							WebsocketCloseReason.find((v) => v.code === data.code)?.resume
						) {
		...
		return new Promise<string>((resolve) => {
			this.bot.request
				...
				.then((res) => {
					...
					let realUrl = 'wss://qqbot.admilk.top/?url=' + res.data.url
					this.wsUrl = realUrl
					resolve(realUrl)
				})
		})
